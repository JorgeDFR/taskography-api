...
class Delfi(PDDLPlanner):
    ...
    def _get_cmd_str(self, dom_file, prob_file, timeout):
        ...
        cmd_str = f"docker run --privileged -it --rm -v {probdom_dir}:/problem -w /problem {DOCKER_IMAGE} {timeout_cmd} {timeout} /root/.planutils/bin/delfi /problem/{dom_fname} /problem/{prob_fname} /problem/delfi.plan --image-from-lifted-task"
        return cmd_str
