...
class LAPKTBFWS(PDDLPlanner):
    ...
    def _get_cmd_str(self, dom_file, prob_file, timeout):
        ...
        cmd_str = f"docker run --privileged -it --rm -v {probdom_dir}:/problem -w /problem {DOCKER_IMAGE} {timeout_cmd} {timeout} bfws --domain /problem/{dom_fname} --problem /problem/{prob_fname} --output /problem/bfws.plan --BFWS-f5 1"
        return cmd_str
