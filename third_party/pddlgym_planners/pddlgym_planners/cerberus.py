...
class Cerberus(PDDLPlanner):
    ...
    def _get_cmd_str(self, dom_file, prob_file, timeout):
        ...
        cmd_str = f"docker run --privileged -it --rm -v {probdom_dir}:/problem -w /problem {DOCKER_IMAGE} {timeout_cmd} {timeout} /root/.planutils/bin/cerberus --alias {self.alias} /problem/{dom_fname} /problem/{prob_fname}"
        return cmd_str
